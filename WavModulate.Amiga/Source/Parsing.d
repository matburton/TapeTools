...
long HeaderByteCount = 61;

proc GetStartLow(*ByteArray_t pByteArray) bool:
    pretend(pByteArray*.ba_pFirstByte + HeaderByteCount - 1, *bool)*
corp;
...
proc GetNextCycleCount(*ByteArray_t pByteArray;
    ...
    if pParseState = nil then
        if pByteArray*.ba_pLastByte - pByteArray*.ba_pFirstByte < HeaderByteCount then
            return nil;
        fi;
        ...
        pParseState*.ps_pNextByte := pByteArray*.ba_pFirstByte + HeaderByteCount;
